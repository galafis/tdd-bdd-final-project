...
class TestProductRoutes(TestCase):
    ...
    def test_create_product(self):
        ...
        self.assertEqual(new_product["image_url"], test_product.image_url) # Check image_url
    ...
    def test_create_product_with_no_name(self):
        ...
        logging.debug("Product no name: %s", product_data)
    ...
    def test_update_product(self):
        ...
        logging.debug(new_product_data)
        new_product_data["description"] = "unknown"
        response = self.client.put(f"{BASE_URL}/{new_product_data['id']}", json=new_product_data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_query_product_list_by_availability(self):
        ...
        response = self.client.get(
            BASE_URL, query_string=f"available={str(test_availability).lower()}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.get_json()
